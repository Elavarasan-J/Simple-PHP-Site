...
$id=(isset($_REQUEST["slider"]) && $_REQUEST["slider"]!="")?(int)$_REQUEST["slider"]:"";
$slider='';
if(isset($_REQUEST["slider"]) && $_REQUEST["slider"]!="") {
    $slider=$db->getSingleRec($db->TB_slider, "slider_id=$id");
}

if($slider==true && $id!=NULL) {
    extract($slider);
    $heading="Update Slider";
    $buttonName='Update Slider';
    $action='class_slider.php?action=update&slider_id='.$id;

    $title=(isset($_POST["title"]) && trim($_POST["title"])!="")?trim($_POST["title"]):$title;
    $featured_title=(isset($_POST["featured_title"]) && trim($_POST["featured_title"])!="")?trim($_POST["featured_title"]):$featured_title;
    ...
    $heading="Add Slider";
    $buttonName='Create Slider';
    $action='class_slider.php?action=add';

    $title=(isset($_POST["title"]) && trim($_POST["title"])!="")?trim($_POST["title"]):'';
    $featured_title=(isset($_POST["featured_title"]) && trim($_POST["featured_title"])!="")?trim($_POST["featured_title"]):'';
    $description=(isset($_POST["description"]) && trim($_POST["description"])!="")?html_entity_decode(trim($_POST["description"])):"";
    $link_name=(isset($_POST["link_name"]) && trim($_POST["link_name"])!="")?trim($_POST["link_name"]):"Donate Now";
...
?>
						...
						<form action="<?php echo $action; ?>" method="post" name="slider" enctype="multipart/form-data">
                                			...
                                			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table1 auto" style="margin:0;">
                                                ...
                                                <tr>
                                                    <td>
                                                        ...
                                                        <input type="text" name="featured_title" id="featured_title" style="width:100%;" value="<?php echo stripslashes($featured_title); ?>" />
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
											...
											<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table1 auto" style="margin:0;">
												<tr>
													...
													<?php if(isset($featured_img) && $featured_img!='') { ?>
														<div class="form-group media-file"><div class="thumb_preview"><img src="<?php echo SITE_PATH.$pathObj->media_library_files_path.$featured_img; ?>" alt=""><a href="class_action.php?action=remove_featured_img&key=slider_id&id=<?php echo $slider_id; ?>&table=slider&page=slider" class="button1 red delete" onclick="return confirm('Are you sure to remove this Slider Image?');"><i class="fa fa-remove"></i> Remove</a></div><input type="hidden" name="featured_img" value="<?php echo $featured_img; ?>"></div>
													<?php } ?>
                                            ...
                                            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table1 auto" style="margin:0;">
                                                ...
                                                <tr>
                                                    <td>
                                                        ...
                                                        <input type="number" step="any" min="1" name="sort_order" id="sort_order" style="width:100px;" value="<?php echo $sort_order; ?>" />
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td>
                                                ...
                                                <tr>
                                                    <td colspan="2"><button type="submit" name="submit" class="button1"  /><i aria-hidden="true" class="fa fa-pencil"></i> <?php echo $buttonName; ?></button> &nbsp; <a href="manage_slider.php" class="button1 red"><i class="fa fa-ban"></i> Cancel</a></td>
                                                </tr>
                                            </table>
                                        </fieldset>
                                    </td>
